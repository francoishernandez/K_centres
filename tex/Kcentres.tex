...
Dans la mesure ou la génération d'instance repose sur des processus aléatoires, les performances peuvent varier d'une instance à l'autre. En conséquence, toutes les mesures renseignées dans cette partie sont des moyennes de 10 relevés de résultats sur des instances différentes.\\

Afin de faciliter l'exploitation de ces mesures, nous avons mis en place un petit programme permettant d'exporter les tests effectués sur un certain nombre de valeurs du nombre de centres $k$ et du nombre de points $n$ dans des fichiers \texttt{.csv}. Ce code est présent dans le package \texttt{Main} dans la classe \texttt{Export.java}.\\
...
On observe que pour des instances uniformes, quelque soit le nombre de centres, le rayon augmente avec le nombre de points. Cela peut s'expliquer par le fait que pour des instances uniformes, plus le nombre de points est élevé, moins il y a de "vide" entre les points, et la taille des disques devra donc être plus importante afin de tout couvrir.\\

Par ailleurs, on remarque que l'algorithme \texttt{Dominant} permet d'avoir de bien meilleurs résultats que l'algorithme \texttt{DeuxApprox}, au détriment de la vitesse de calcul.\\

Enfin, on peut aussi noter que, de façon évidente, pour un nombre de points égal au nombre de centres, le rayon approximé est bien nul.
...
Pour des instances contenant 10 clusters, on observe également une augmentation du rayon en fonction du nombre de points, mais plus légère que pour les instances uniformes. Cela est dû au fait que les points, même plus nombreux, sont concentrés autour de clusters d'une taille fixe.\\

De façon assez logique, plus le nombre de centre est grand, plus le rayon est faible. Effectivement, un nombre de centre plus grand permet de couvrir autant de surface pour un rayon plus faible, et donc de contenir le même nombre d'instances pour un rayon plus faible.\\

Par ailleurs, comme pour les instances uniformes, on remarque que l'algorithme \texttt{Dominant} permet d'avoir de bien meilleurs résultats que l'algorithme \texttt{DeuxApprox}, au détriment de la vitesse de calcul.
...
On observe que pour les instances uniformes le rayon est supérieur à celui pour les instances en clusters. Cela s'explique par le fait que pour ces dernières, les points sont regroupés en clusters, et donc par définition répartis de façon moins éparse dans l'espace.\\

\subsection{Performances de la Descente}

L'algorithme \texttt{Descente} doit être évalué différemment. En effet, il est utilisé pour améliorer des résultats obtenus avec d'autres algorithmes. On peut donc essayer d'observer son pouvoir d'amélioration. Pour fonctionner, il faut donner à l'algorithme un minimum d'opportunités de permutation. On choisira donc des instances à k=10 centres. En figure 4 nous pouvons observer ses effets sur l'algorithme \texttt{DeuxApprox} : c'est assez efficace ! On diminue le rayon de près de 10, et de manière assez notablement constante selon le nombre de noeuds. Cependant sur \texttt{Dominant}, en figure 5, c'est moins efficace. L'algorithme est déjà très performant, c'est donc difficile d'améliorer encore les résultats (c'est même presque toujours impossible pour des nombres faibles de points).\\
...
xlabel={Nombre de points},
ylabel={Rayon},
legend entries={Avant descente, Après descente},
legend style={at ={(0.9,0.9)}},
height= 10cm,
width = 12cm
]

% k =10, cluster 10, 15, deuxApprox, moyennés sur 10 expériences
...
\caption{Performances de DeuxApprox avant et après utilisation de Descente.}
\end{center}
\end{figure}
...
\caption{Performances de Dominant avant et après utilisation de Descente.}
\end{center}
\end{figure}
